...
public partial class DownlineAutoships : System.Web.UI.Page
{
    ...
    public List<ReportDataNode> FetchReportData()
    {
        ...
        foreach(DataRow row in table.Rows)
        {
            ...
            node.NextRunDate            = Convert.ToDateTime(row["NextRunDate"]);
            ...
            switch(Request.QueryString["action"])
            {
                case "fetch":
                    ...
                    foreach(var record in nodes)
                    {
                        ...
                        html.AppendFormat("<td><a href='Profile.aspx?id={0}' title='View profile'>{0}</a></td>", record.CustomerID);
                        html.AppendFormat(@"
                        ...
                        html.AppendFormat("<td>{0:M/d/yyyy}</td>", record.NextRunDate);
